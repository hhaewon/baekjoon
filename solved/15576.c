...
int main()
{
    ...
    cd *A = (cd *)malloc(sizeof(cd) * size);
    cd *B = (cd *)malloc(sizeof(cd) * size);
    ...
    {
        // printf("A %d %lf\n", i, (double)(Astr[i] - '0'));
        A[i / 2] = (double)((Astr[length1 - i - 1] - '0') * 10 + Astr[length1 - i - 2] - '0');
    }
    if (length1 % 2 == 1)
    {
        A[length1 - 1] = (double)(Astr[0] - '0');
    }
    for (int i = 0; i < length2; i += 2)
    {
        B[i / 2] = (double)(Bstr[length2 - i - 1] - '0') * 10 + Bstr[length2 - i - 2];
    }
    if (length2 % 2 == 1)
    {
        B[length2 - 1] = (double)(Bstr[0] - '0');
    }
    for (int i = length1; i < size; i++)
    {
        A[i] = 0;
    }
    for (int i = length2; i < size; i++)
    {
        B[i] = 0;
    }
    // printf("A : ");
    ...
    for (int i = 0; i < size; i++)
    {
        long long int temp = round(creal(CIfft[i])) + carry;
        if (temp)
        {
            len = i;
        }
